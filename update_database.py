...
1. FASE 1: Descargar Excels (precios/download/)
2. FASE 2: Actualizar BD (precios/update/ y macro/update/)
...
# Configuración
REPORTE_FILE = "update_database.txt"
...
def descubrir_scripts_download() -> Dict[str, List[Path]]:
    ...
    precios_productos_dir = Path("precios/download/productos")
    if precios_productos_dir.exists():
        for script_file in precios_productos_dir.glob("*.py"):
            if script_file.name != "__init__.py" and not script_file.name.startswith("_"):
                scripts['precios_productos'].append(script_file)
    
    # Scripts de precios/servicios/download
    precios_servicios_dir = Path("precios/download/servicios")
...
def descubrir_scripts_update() -> Dict[str, List[Path]]:
    ...
    precios_productos_dir = Path("precios/update/productos")
    if precios_productos_dir.exists():
        for script_file in precios_productos_dir.glob("*.py"):
            if script_file.name != "__init__.py" and not script_file.name.startswith("_"):
                scripts['precios_productos'].append(script_file)
...
def ejecutar_todas_actualizaciones() -> None:
    ...
    Genera reporte en update_database.txt
    """
    print("=" * 80)
